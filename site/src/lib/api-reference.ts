  ...
  pptx: [
    {
      ...
      methods: [
        ...
        { sig: 'load(source: string | ArrayBuffer): Promise<void>', desc: 'Load a Viewer-owned URL or ArrayBuffer and render the first slide.' },
    ...
    },
    {
      ...
      methods: [
        ...
        { sig: 'load(source: string | ArrayBuffer): Promise<void>', desc: 'Load a Viewer-owned deck and render the first window.' },
  ...
  docx: [
    {
      ...
      methods: [
        ...
        { sig: 'load(source: string | ArrayBuffer): Promise<void>', desc: 'Load a Viewer-owned URL or ArrayBuffer and render the first page.' },
    ...
    },
    {
      ...
      methods: [
        ...
        { sig: 'commentAnchorRanges(): readonly CommentAnchorRange[]', desc: 'Logical comment ranges that can be joined to rendered text runs. Describes the layout currently in view: while `layoutComplete` is false these cover only the pages laid out so far, so re-read after each progressive publication (or await `whenLayoutComplete()`).' },
        ...
        { sig: 'revisionAnchorRanges(): readonly RevisionAnchorRange[]', desc: 'Logical tracked-change ranges that can be joined to rendered text runs. Same progressive-layout contract as `commentAnchorRanges()`.' },
        ...
        { sig: 'setLayoutView(view?: { showTrackedChanges?, currentDate? }): void', desc: 'Select the layout variant the geometry accessors (pageCount, pageSize, bookmark lookup) read from, matching the variant being rendered. Viewers call this when the markup view toggles.' },
    ...
    },
    {
      ...
      methods: [
        ...
        { sig: 'load(source: string | ArrayBuffer): Promise<void>', desc: 'Load a Viewer-owned document and render the first window.' },
        ...
        { sig: 'setShowTrackedChanges(value: boolean): void', desc: 'Switch between the final view (false, default) and the tracked-change markup view (true) at runtime. Every mounted page re-renders against the selected layout variant; the page count follows the new pagination.' },
        ...findMethods('DocxMatchLocation'),
        { sig: 'setScale(scale: number): void', desc: 'Set the absolute zoom scale at runtime (clamped to zoomMin/zoomMax). Flicker-free. View-only.' },
        { sig: 'relayout(): void', desc: 'Force a re-fit + re-mount of the visible window. Called automatically after load / resize / zoom; use it when the container resizes in a way a ResizeObserver cannot observe (e.g. a late web-font load). Idempotent.' },
        { sig: 'get pageCount(): number', desc: 'Total pages (0 until loaded).' },
